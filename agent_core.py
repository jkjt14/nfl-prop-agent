# agent_core.py
import os, re, math, csv, time, datetime as dt, logging
...
def sanity_line_ok(market_key: str, line: float) -> bool:
    # RELAXED guardrails (quick fix to avoid throwing out lines)
    if line is None:
        return False
    return line >= 0.0
...
# -------------------- Best offer per player (robust) --------------------
def _player_name_match(desc: str, player_norm: str) -> bool:
    """Loose but safe: handles initials, extra punctuation, etc."""
    if not desc:
        return False
    ...
    if not pt:
        return False
    # Require last name match; first can be full or initial
    last_ok = pt[-1] in dt
    if len(pt) >= 2:
        first = pt[0]
...
def best_offer_for_player(
    ...
    """
    Returns (book, line, price) for best available outcome for player+market+side.
    First tries target_books; if none found, falls back to ANY book.
    """
            ...
            for mk in bm.get("markets", []) or []:
                ...
                for outc in mk.get("outcomes", []) or []:
                    ...
                    if best_local is None:
                        ...
                        # OVER: lower line then better odds; UNDER: higher line then better odds
                        if side == "OVER":
...
def scan_edges(
    ...
    *,
    ...
    for event_id, ev in event_map.items():
        ...
        # ---- TEAM FILTER (with safe fallback & optional bypass) ----
